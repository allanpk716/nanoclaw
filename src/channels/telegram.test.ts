...
describe('TelegramChannel', () => {
  ...
  describe('sendMessage', () => {
    ...
    it('stops typing indicator before sending message', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();
      ...
      // Send message - should stop typing
      await channel.sendMessage('tg:100200300', 'Reply');

      // Advance time - should not refresh anymore
  ...
  describe('setTyping', () => {
    ...
    it('starts refresh interval when typing starts', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();
    ...
    it('stops refresh interval when typing stops', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();
    ...
    it('clears existing interval when starting typing again', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();
      ...
      // Start typing again (should clear old interval)
      await channel.setTyping('tg:100200300', true);

      // Only initial calls, no refresh yet
      expect(currentBot().api.sendChatAction).toHaveBeenCalledTimes(2);

      // Advance 4 seconds from second start
      await vi.advanceTimersByTimeAsync(4000);

      // Should refresh once from second start
      expect(currentBot().api.sendChatAction).toHaveBeenCalledTimes(3);
    });

    it('stops typing on error during refresh', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();
      ...
      // Advance more time - should not try again (interval cleared)
      await vi.advanceTimersByTimeAsync(4000);
      expect(currentBot().api.sendChatAction).toHaveBeenCalledTimes(2);
    });
    ...
    it('handles initial typing indicator failure gracefully', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();
      ...
      // Should not start interval if initial send failed
      await vi.advanceTimersByTimeAsync(10000);
      expect(currentBot().api.sendChatAction).toHaveBeenCalledTimes(1);
    });
    ...
    it('clears all intervals on disconnect', async () => {
      ...
      // Start typing in multiple chats
      await channel.setTyping('tg:100200300', true);
      await channel.setTyping('tg:200300400', true);

      expect(currentBot().api.sendChatAction).toHaveBeenCalledTimes(2);

      // Disconnect should clear all intervals
      await channel.disconnect();

      // Advance time - no more refreshes
      await vi.advanceTimersByTimeAsync(10000);
      expect(currentBot().api.sendChatAction).toHaveBeenCalledTimes(2);
    });
  });
